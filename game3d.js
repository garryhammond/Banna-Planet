...
function groundHole(n,s=1){
  ...
  const pit=new THREE.Mesh(new THREE.CircleGeometry(.43,36),pitMat);pit.rotation.x=-Math.PI/2;pit.position.y=.032;pit.scale.z=.72;pit.renderOrder=3;g.add(pit);
  const innerWall=new THREE.Mesh(new THREE.TorusGeometry(.425,.052,8,36),new THREE.MeshStandardMaterial({color:0x3a2114,roughness:1}));innerWall.rotation.x=Math.PI/2;innerWall.position.y=.038;innerWall.scale.z=.72;innerWall.castShadow=true;g.add(innerWall);
  for(let i=0;i<7;i++){const a=[.18,.76,1.58,2.4,3.2,4.25,5.4][i],r=.43,clod=new THREE.Mesh(new THREE.DodecahedronGeometry(.045+(i%3)*.009,0),i===2||i===5?rockMat:soilMat);clod.position.set(Math.cos(a)*r,.052,Math.sin(a)*r*.72);clod.scale.set(1.25,.48,.78);clod.rotation.y=-a+i*.29;clod.castShadow=true;g.add(clod);}
...
function fallInHole(hole){
  ...
  holeFallTimer=1.4;holeRespawnPoint=clearRecoveryPoint(targetN.clone().applyAxisAngle(respawnAxis,.08),targetN);toast.textContent='WHOA-A-A!';document.documentElement.dataset.lastHazard='hole';
}
updateHUD();
